...
namespace dftfe
{
    ...
    const bool vselfPerturbationUpdateForStress)
  {
    ...
    if (d_dftParamsPtr->applyExternalPotential)
      {
        if (d_dftParamsPtr->externalPotentialType == "CEF")
          {
            // Dont do anything
          }
        else if (d_dftParamsPtr->externalPotentialType == "CPD-APD")
          {
            locateConstrainedPotentialPinnedNodes(
              (d_dftParamsPtr->zCoordinateL - 0.5) *
                d_domainBoundingVectors[2][2],
              (d_dftParamsPtr->zCoordinateR - 0.5) *
                d_domainBoundingVectors[2][2],
              d_dftParamsPtr->potentialValueL,
              d_dftParamsPtr->potentialValueR,
              false,
              d_dftParamsPtr->localizedWidth,
              d_dofHandlerPRefined,
              d_constraintsPRefined, // Periodic constrinats must be filled here
              d_constraintsForTotalPotentialElectro);
          }
        else if (d_dftParamsPtr->externalPotentialType == "CPD-PPD")
          {
            locateConstrainedPotentialPinnedNodes(
              (d_dftParamsPtr->zCoordinateL - 0.5) *
                d_domainBoundingVectors[2][2],
