...
namespace dftfe
{
  ...
  forceClass<memorySpace>::computeAtomsForces(
    ...
    const bool   externalElectricFieldOrSawToothPotentialFlag,
    const double externalElectricFieldOrSawToothPotentialSlope)
  {
    ...
    if (externalElectricFieldOrSawToothPotentialFlag)
      for (dftfe::uInt iAtom = 0; iAtom < d_dftParams.natoms; iAtom++)
        {
          d_globalAtomsForces[iAtom * 3 + 2] +=
            -atomLocations[iAtom][1] *
            externalElectricFieldOrSawToothPotentialSlope;
        }
